...
def test(loader, neg_sampler, split_mode):
    ...
    for pos_batch in loader:
        ...
        for idx, neg_batch in enumerate(neg_batch_list):
            ...
            resutls = evaluator.eval(input_dict)
            perf_list.append(resutls[metric])
            hits_list.append(resutls['hits@10'])
    ...
    hits_metric = float(torch.tensor(hits_list).mean())

    return perf_metric, hits_metric
...
NUM_NEIGHBORS = 10
USE_EDGE_TYPE = args.use_edge_type
...
for run_idx in range(NUM_RUNS):
    ...
    val_perf_list, val_time_list, train_time_list, val_hits_list = [], [], [], []
    ...
    for epoch in range(1, NUM_EPOCH + 1):
        ...
        perf_metric_val, hits_val = test(
            val_loader, neg_sampler, split_mode="val")
        end_val = timeit.default_timer()
        print(
            f"\tValidation {metric}: {perf_metric_val: .4f}; Hits@10: {hits_val}")
        print(f"\tValidation: Elapsed time (s): {end_val - start_val: .4f}")
        val_perf_list.append(perf_metric_val)
        val_hits_list.append(hits_val)
    ...
    perf_metric_test, hits_metric_test = test(
        test_loader, neg_sampler, split_mode="test")

    print(f"INFO: Test: Evaluation Setting: >>> ONE-VS-MANY <<< ")
    print(
        f"\tTest: {metric}: {perf_metric_test: .4f}; Hits@10: {hits_metric_test}")
    ...
    save_results({'model': MODEL_NAME,
                  ...
                  'validation Hits@10s': val_hits_list,
                  ...
                  f'Best val Hits@10': np.max(np.array(val_hits_list)),
                  f'test {metric}': perf_metric_test,
                  'test Hits@10': hits_metric_test,
                  },
                 results_filename)
