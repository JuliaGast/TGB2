...
def get_args():
    ...
    parser.add_argument('--num_edgeType', type=int,
                        default=0, help='num of edgeType')
    ...
    parser.add_argument('--num_run', type=int,
                        help='Number of iteration runs', default=5)
...
def test(data, test_mask, model, neg_sampler, split_mode):
    ...
    test_subgraphs = pre_compute_subgraphs(
        args, g, df, mode='test' if split_mode == 'test' else 'valid', negative_sampler=neg_sampler, split_mode=split_mode)
    perf_list = []
    ...
    pbar.set_description('%s mode with negative samples %d ...' %
                         (split_mode, neg_samples))
    ...
    for ind in range(len(test_loader)):
        ...
        inputs, subgraph_node_feats, cur_inds = get_inputs_for_ind(
            test_subgraphs, 'test' if split_mode == 'test' else 'tgb-val', cached_neg_samples, neg_samples, node_feats, edge_feats, cur_df, cur_inds, ind, args)

        loss, pred, edge_label = model(
            inputs, neg_samples, subgraph_node_feats)
...
for run_idx in range(NUM_RUNS):
    ...
    model = link_pred_train(model.to(args.device), args,
                            g, df, node_feats, edge_feats)

    dataset.load_val_ns()

    # Validation ...
    start_val = timeit.default_timer()
    perf_metrics_val_mean, perf_metrics_val_std, perf_list_val = test(data.to(
        args.device), test_mask, model.to(args.device), neg_sampler, split_mode='val')
    ...
    perf_metrics_test_mean, perf_metrics_test_std, perf_list_test = test(data.to(
        args.device), test_mask, model.to(args.device), neg_sampler, split_mode='test')
