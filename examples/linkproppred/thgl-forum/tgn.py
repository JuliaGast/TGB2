...
for run_idx in range(NUM_RUNS):
    ...
    for epoch in range(1, NUM_EPOCH + 1):
        ...
        perf_metric_val, hits_metric_val = test(
            val_loader, neg_sampler, split_mode="val")
        end_val = timeit.default_timer()
        print(
            f"\tValidation {metric}: {perf_metric_val: .4f}; Hits@10: {hits_metric_val: .4f}")
        print(f"\tValidation: Elapsed time (s): {end_val - start_val: .4f}")
        val_perf_list.append(perf_metric_val)
        val_hits_list.append(hits_metric_val)
    ...
    perf_metric_test, hits_metic_test = test(
        test_loader, neg_sampler, split_mode="test")

    print(f"INFO: Test: Evaluation Setting: >>> ONE-VS-MANY <<< ")
    print(
        f"\tTest: {metric}: {perf_metric_test: .4f}, Hits@10: {hits_metic_test: .4f}")
    ...
    save_results({'model': MODEL_NAME,
                  ...
                  'validation Hits@10': val_hits_list,
                  ...
                  'Best val Hits@10': np.max(np.array(val_hits_list)),
                  f'test {metric}': perf_metric_test,
                  'test Hits@10': hits_metic_test,
                  },
                 results_filename)
